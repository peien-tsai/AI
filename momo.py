    ...
    for k in board :    #上列標題的所有內容    #####品牌旗艦店的都先跳過######
        ...
        for l in left:
            for n in l.values():
                ...
                # print('共'+str(lastpage)+'頁')
                # for j in range(lastpage): #翻頁
                #     print("第{}頁".format(j+1))
                #     driver.implicitly_wait(5)
                #     print(n)
                #     print(n + '&showType=chessboardType&p_pageNum={}'.format(j+1))
                driver.get(n)
                page = driver.page_source
                ...
                for m in soup2.select('div[class="prdListArea bt770class"] li'):    #soup2.select('div[class="prdListArea bt770class"] a[class="prdUrl"]')
                    ...
                    try:
                        ...
                        obj['desc'] = m.a['title']
                    try:
                        for n in soup4.select('div[class = "vendordetailview specification"] tr'):
                            obj[n.th.text] = n.td.text  #框框裡所有內容
                    except :
                        pass
                    small = []
            ...
            print(output)
            print(len(output))
            # with open(momo + "/" + "{}_{}".format(i.a.text,[x for x in l.keys()][0]) + ".json", "w",encoding="utf-8") as f:
