...
for k in soup.select('div[class="contenttop topArea"] ')[2].select('li') :    #上列標題的所有內容    #####品牌旗艦店的都先跳過######
    ...
    for l in left:
        for n in l.values():
            ...
            if not os.path.exists(momo_sheet):
                os.mkdir(momo_sheet)
            with open(momo + "/" + "{}".format(k.a.text) + "/" + "{}_{}".format(k.a.text,[x for x in l.keys()][0]) + ".json", "w",encoding="utf-8") as f:
